...
#'
#' @description Fit Weibull Gap Model using direct maximization of marginal likelihood with optim.
#'
...
joint.weibull.gap.optim <- function(data, covar.names="trt", GH = 30, method = "L-BFGS-B"){
	...
	par.start = c(rep(0,length(covar.names)*2 + 6))
	names(par.start) <- c(paste0(rep(c("beta","eta"),2),rep(c("R","D"), each = 2)),
		          "theta", "alpha1",
		          paste0(covar.names,c("R","D")))
	optim.out <-
		optim(par = par.start,
		      fn = marg.likelihood.weibull.gap,
		      data = data,
		      GH = GH,
