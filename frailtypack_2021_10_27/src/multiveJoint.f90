...
! End Inputs    
!-------------------------------------------------
! Unpacked Versions of Initialization Variables
...
!-------------------------------------------------
! Variables Used in Initialization with Joint Models
    integer::np1,np2,np3,np4,np2joint
    double precision,dimension(:),allocatable::b01,b02,b03,b04,b02joint
    double precision,dimension(4)::paraweibjoint
    integer, dimension(2)::critCVjoint, typeofequidistjoint, nbintervjoint
    integer, dimension(4)::mtaillejoint	
    integer, dimension(3)::paratpsjoint, ngjoint
    
    double precision,dimension(4)::shape_weib,scale_weib    
    integer::noVar1,noVar2,noVar3,noVar4
    integer, dimension(2)::noVar2joint, indicesjoint
        ! individual indicators for no variables in initialization models
    double precision,dimension(:),allocatable::linearpredGjoint
    integer,dimension(:),allocatable::ordretmpjoint
    double precision,dimension(:,:),allocatable::tempdcjoint
    double precision,dimension(:),allocatable::wtsvecjoint
    double precision,dimension(:),allocatable::ttUjoint
	
    double precision,dimension(100,1)::x1joint
    double precision,dimension(100,3,1)::lamjoint
    double precision,dimension(100,1)::xSu1joint
    double precision,dimension(100,3,1)::sujoint

    double precision,dimension(100)::x2joint
    double precision,dimension(100,3)::lam2joint
    double precision,dimension(100)::xSu2joint
    double precision,dimension(100,3)::su2joint

!-------------------------------------------------
! Data Summaries and Temporary Copies of Variables
...
!     double precision,dimension(100)::xSu1s,xSu2s
    ...
    double precision,dimension(0:100,1)::BetaTpsMatMulDC
...
! (2) Parameter Initialization
    ...
    select case(typeof)
        case(0) ! splines
            ...
            np3 = nz0 + 2 + nva30 + 1
            np4 = nz0 + 2 + nva40 + 1
            np2joint = nz0 + nz0 + 4 + nva10 + nva20 + 2 ! 2*nz0 + 4 baseline hazard parameters, frailty variance, and link (alpha)
        !case(1) ! peicewise
        !    k0=0.d0
...
! With Initialization
    if(initialize.eq.1)then
        ...
        qorder0Mul=0
        BetaTpsMatMul=0.d0
        BetaTpsMatMulDC=0.d0
! ------------------------------------------------
! Initialize First Recurrent Event Model 
        nzsha = nz0
        kappa1 = k0(1)
        EPS(1) = 1.d-3
        EPS(2) = 1.d-3
...
!         allocate(x1Outs(mt1),x2Outs(mt1),lamOuts(mt1,3),lam2Outs(mt1,3))
        ...
        allocate(linearpreds(nsujet0),martingaleCoxs(nsujet0),times(nbinterv0+1))

        allocate(filtretps0Mul(nva10))
        filtretps0Mul = 0
        
        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)'multiveJoint.f90:: ...initializing recurrent 1.'
        close(1) 
        ...
        xTOuts,lamTOuts,xSuTs,suTOuts,typeof0,equidistant0,nbinterv0,mt1, &    
...
!         deallocate(x1Outs,x2Outs,lamOuts,lam2Outs)
        ...
        !write(*,*)'========= number of parameters ',np1
        !write(*,*)'========= number of rows ',nsujet0
        !write(*,*)'========= number of groups ',ng0,nva10,size(vax0),noVar1,ag0
        !write(*,*)' Convergence Criteria R1: istop = ',istop
        !write(*,*)'Recurrent 1 Initialized Parameters:', b01
        !write(*,*)''
        if(typeof == 0)then
            if(irep.eq.0)then
...
! ------------------------------------------------
! Initialize Second Recurrent Event Model (META)
        if(event2_ind0.eq.0)then
            !write(*,*)'Second recurrent event not present. No initialization for second recurrent event.'
        else
            b03 = 0.25d0
            nzsha = nz0
            kappa1 = k0(3)
            ...
            str00=1
            allocate(H_hess0(np3,np3),HIH0(np3,np3),zis(nzsha+6))!ziOut1
            !         allocate(x1Outs(mt3),x2Outs(mt3),lamOuts(mt3,3),lam2Outs(mt3,3))
            allocate(xTOuts(mt1,1),lamTOuts(mt1,3,1))!en plus
            allocate(linearpreds(nsujetmeta0),martingaleCoxs(nsujetmeta0),times(nbinterv0+1))
            allocate(filtretps0Mul(nva30))
            filtretps0Mul = 0

            open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
            write(1,*)'multiveJoint.f90:: ...initializing recurrent 2.'
            close(1) 
            ...
            xTOuts,lamTOuts,xSuTs,suTOuts,typeof0,equidistant0,nbinterv0,mt3, &    
            ni,cpts,ier,k0s,ddls,istop,shapeweibs,scaleweibs,100,zis,Resmartingales,martingaleCoxs,&
            frailtypreds,frailtyvars,frailtysds,linearpreds,times,0,tt10,0, &
            timedepMul,nbinnerknots0Mul,qorder0Mul,filtretps0Mul,BetaTpsMatMul,EPS,20)
...
!            deallocate(x1Outs,x2Outs,lamOuts,lam2Outs)
            ...
            !write(*,*)'=============== estimation R2 ================='
            !write(*,*)'========= typeof = (0:Splines, 1:Piecewise, 2:Weibull) ',typeof0    
            !write(*,*)'========= number of parametres ',np3
            !write(*,*)'========= number of rows ',nsujetmeta0
            !write(*,*)'========= number of groups ',ng0
            !write(*,*)'Convergence Criteria Recurrent 2: istop = ',istop
            !write(*,*) 'Recurrent 2 Initialized Parameters:', b03
            !write(*,*)''
...
!------------------------------------------------ 
!Initialize First Terminal Event Model: DC

        !Initialization using just terminal event data
        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)'multiveJoint.f90:: Initializing terminal 1.'
        write(1,*)'multiveJoint.f90:: b02joint = ', b02joint
        close(1) 

        mtaillejoint = 100
        ngjoint(1) = ng0 ! distinct groups
        ngjoint(2) = 1 ! distinct subgroups
        ngjoint(3) =  1! distinct strata
        nbintervjoint(1) = 0
        nbintervjoint(2) = 0
        typeofequidistjoint(1) = typeof0
        typeofequidistjoint(2) = equidistant0
        indicesjoint(1) = 1
        indicesjoint(2) = 3
        paratpsjoint = 0
        kappa1 = k0(2)
        ...
        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)'multiveJoint.f90:: ngjoint = ', ngjoint
        write(1,*)'multiveJoint.f90:: typeofequidistjoint = ', typeofequidistjoint
        write(1,*)'multiveJoint.f90:: indicesjoint = ', indicesjoint
        write(1,*)'multiveJoint.f90:: kappa1 = ', kappa1
        write(1,*)'multiveJoint.f90:: EPS = ', EPS
        write(1,*)'multiveJoint.f90:: paratpsjoint = ', paratpsjoint
        close(1) 

        allocate(str00(ng0))
        allocate(H_hess0(np2joint,np2joint),HIH0(np2joint,np2joint))
        allocate(zidc(-2:(nzdc+3)))
        allocate(zi(-2:(nzloco+3)))
        allocate(filtretps0Mul(nva10 + nva20))
        allocate(linearpredGjoint(ng0))
        allocate(ordretmpjoint(nsujet0))
        allocate(wtsvecjoint(ng0))
        allocate(tempdcjoint(ng0,2))
        allocate(ttUjoint(nsujet0))

        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)'multiveJoint.f90:: Initialization variables allocated.'
        close(1) 
        ...
        zidc=0.d0
        filtretps0Mul = 0
        linearpredGjoint = 0.d0
        ordretmpjoint = 0
        wtsvecjoint = 1.d0
        tempdcjoint(1:ng0,1)= 0.d0
        tempdcjoint(1:ng0,2)= tt1dc0(1:ng0)
        ttUjoint(1:nsujet0) = tt10(1:nsujet0)

        !open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        !write(1,*)'multiveJoint.f90:: tempdcjoint = ', tempdcjoint
        !close(1) 
		
        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)'multiveJoint.f90:: size(ngjoint)', size(ngjoint)
        write(1,*)'multiveJoint.f90:: size(str00)', size(str00)
        write(1,*)'multiveJoint.f90:: size(tt00)', size(tt00)
        write(1,*)'multiveJoint.f90:: size(tt10)', size(tt10)
        write(1,*)'multiveJoint.f90:: size(ic0)', size(ic0)
        write(1,*)'multiveJoint.f90:: size(groupe0)', size(groupe0)
        write(1,*)'multiveJoint.f90:: size(groupe0dc)', size(groupe0dc)
        write(1,*)'multiveJoint.f90:: size(tt0dc0)', size(tt0dc0)
        write(1,*)'multiveJoint.f90:: size(tt1dc0)',size(tt1dc0)
        write(1,*)'multiveJoint.f90:: size(icdc0)', size(icdc0)
        write(1,*)'multiveJoint.f90:: size(tempdcjoint)', size(tempdcjoint)
        write(1,*)'multiveJoint.f90:: size(vax0)', size(vax0)
        write(1,*)'multiveJoint.f90:: size(vaxdc0)', size(vaxdc0)
        write(1,*)'multiveJoint.f90:: size(noVar2joint)', size(noVar2joint)
        write(1,*)'multiveJoint.f90:: size(wtsvecjoint)', size(wtsvecjoint)
        write(1,*)'multiveJoint.f90:: size(b02joint)', size(b02joint)
        write(1,*)'multiveJoint.f90:: size(H_hess0)', size(H_hess0)
        write(1,*)'multiveJoint.f90:: size(HIH0)', size(HIH0)
        write(1,*)'multiveJoint.f90:: size(LCV)', size(LCV)
        write(1,*)'multiveJoint.f90:: size(x1Out)',  size(x1Out)
        write(1,*)'multiveJoint.f90:: size(lamOut)',size(lamOut)
        write(1,*)'multiveJoint.f90:: size(xSu1)',  size(xSu1)
        write(1,*)'multiveJoint.f90:: size(suOut)',size(suOut)
        write(1,*)'multiveJoint.f90:: size(x2Out)', size(x2Out)
        write(1,*)'multiveJoint.f90:: size(lam2Out)', size(lam2Out)
        write(1,*)'multiveJoint.f90:: size(xSu2)',  size(xSu2)
        write(1,*)'multiveJoint.f90:: size(su2Out)', size(su2Out)
        write(1,*)'multiveJoint.f90:: size(typeofequidistjoint)', size(typeofequidistjoint)
        write(1,*)'multiveJoint.f90:: size(cptEvent)', size(cptEvent)
        write(1,*)'multiveJoint.f90:: size(critCVjoint)', size(critCVjoint)
        write(1,*)'multiveJoint.f90:: size(paraweibjoint)', size(paraweibjoint)
        write(1,*)'multiveJoint.f90:: size(frailtyEstimates)', size(frailtyEstimates)
        write(1,*)'multiveJoint.f90:: size(linearpred)', size(linearpred)
        write(1,*)'multiveJoint.f90:: size(linearpreddc)', size(linearpreddc)
        write(1,*)'multiveJoint.f90:: size(zi)', size(zi)
        write(1,*)'multiveJoint.f90:: size(time)',size(time)
        write(1,*)'multiveJoint.f90:: size(timedc)', size(timedc)
        write(1,*)'multiveJoint.f90:: size(linearpredGjoint)', size(linearpredGjoint)
        write(1,*)'multiveJoint.f90:: size(indicesjoint)', size(ordretmpjoint)
        write(1,*)'multiveJoint.f90:: size(ordretmpjoint)', size(ordretmpjoint)
        write(1,*)'multiveJoint.f90:: size(paratpsjoint)', size(paratpsjoint)
        write(1,*)'multiveJoint.f90:: size(filtretps0Mul)', size(filtretps0Mul)
        write(1,*)'multiveJoint.f90:: size(BetaTpsMatMul)',size(BetaTpsMatMul)
        write(1,*)'multiveJoint.f90:: size(BetaTpsMatMulDC)',size(BetaTpsMatMul)
        write(1,*)'multiveJoint.f90:: size(EPS)', size(EPS)
        close(1) 

        Call joint(nsujet0,ngjoint,str00&
            ,ng0,nz0,kappa1 & !lignedc = ng0
            ,tt00,tt10,ic0,groupe0&
            ,groupe0dc, 0 & !fam0 = 0
            ,tt0dc0,tt1dc0,icdc0 &
            ,tempdcjoint,0,nva10,vax0,nva20,vaxdc0& !icdc00=0
            ,0,noVar2joint &!vaxdc00 = 0
            ,wtsvecjoint, maxit0 & 
            ,np2joint,b02joint,H_hess0,HIH0,resOut,LCV &
            ,x1joint,lamjoint,xSu1joint,sujoint,x2joint,lam2joint,xSu2joint,su2joint &
            ,typeofequidistjoint,nbinterv0,mtaillejoint,cptEvent & !mtaillejoint = 100, counts = cptEvent
            ,critCVjoint,paraweibjoint &
            ,frailtyEstimates,linearpred,linearpreddc,zi,time,timedc &
            ,linearpredGjoint,0,0 &!incens0=0, typeJoint0 = 0
            ,indicesjoint,ttUjoint,ordretmpjoint,1,1 & !initialize = 1, logNormal0 = 1, ttU0 = tt10 for right-censored survival data
            ,paratpsjoint,filtretps0Mul,BetaTpsMatMul,BetaTpsMatMulDC, EPS, 20) !paratpsjoint = 0, nbgh = 20
...
!		     double precision,dimension(mtaille(4),3)::su2Out
        ...
        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)'multiveJoint.f90:: Initialization model fit.'
        write(1,*)'multiveJoint.f90:: b02joint = ', b02joint
        close(1) 

        deallocate(filtretps0Mul)
        deallocate(H_hess0,HIH0,str00)
        deallocate(zi,zidc)
        deallocate(linearpredGjoint,ordretmpjoint,wtsvecjoint,tempdcjoint,ttUjoint)
        
        ! Reset some values to 0 if they were borrowed to use in initialization model
        x1Out= 0.d0
        lamOut= 0.d0
        xSu1 = 0.d0
        suOut=0.d0
        ...
        timedc=0.d0
        cptEvent=0
        frailtyEstimates=0.d0
        !zi=0.d0

        ! Save Convergence Code
        critCV(4) = critCVjoint(2)

        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)'multiveJoint.f90:: Terminal 1 initialized.'
        write(1,*)'multiveJoint.f90:: b02joint = ', b02joint
        write(1,*)'multiveJoint.f90:: critCVjoint = ', critCVjoint
        close(1) 
        !write(*,*)''
        !write(*,*)'=============== Estimation Terminal 1 ================='
        !write(*,*)'========= typeof = (0:Splines, 1:Piecewise, 2:Weibull) ',typeof0    
        !write(*,*)'========= number of parameters ',np2
        !write(*,*)'========= number of subjects ',ng0
        !write(*,*)'========= number of groups ',ng0
        !write(*,*)' Critere de convergence istop dc',istop
        !write(*,*) 'Terminal 1 Initialized Parameters:', b02
        !write(*,*)''
        if(typeof == 0)then
            if(irep.eq.0)then
...
!------------------------------------------------
! Initialize Second Terminal Event Model: DC2
        ...
        allocate(str00(ng0))
        str00=1
        allocate(H_hess0(np4,np4),HIH0(np4,np4),zis(nzsha+6))
!         allocate(x1Outs(mt2),x2Outs(mt2),lamOuts(mt2,3),lam2Outs(mt2,3))
        allocate(xTOuts(mt4,1),lamTOuts(mt4,3,1))!en plus
        allocate(linearpreds(ng0),martingaleCoxs(ng0),times(nbinterv0+1))
        allocate(filtretps0Mul(nva40))
        filtretps0Mul = 0

        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)'multiveJoint.f90:: Initializing terminal 2.'
        close(1) 
        Call frailpenal(ng0,ng0,1,1,1, & ! 0 Cox proportional hazards model
        nzsha,kappa1,tt0dc0,tt1dc0,icdc20,groupe0dc,nva40,str00,vaxdc20, &
        ag0,noVar4,maxit00,irep,np4,b04,H_hess0,HIH0,resOut,LCVs, &
        xTOuts,lamTOuts,xSuTs,suTOuts,typeof0,equidistant0,nbinterv0,mt4, &    
        ni,cpts,ier,k0s,ddls,istop,shapeweibs,scaleweibs,mt14,zis,Resmartingales,martingaleCoxs,&
        frailtypreds,frailtyvars,frailtysds,linearpreds,times,0,tt10,0, &
        timedepMul,nbinnerknots0Mul,qorder0Mul,filtretps0Mul,BetaTpsMatMul,EPS,20)
...
!         deallocate(x1Outs,x2Outs,lamOuts,lam2Outs)
        ...
        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)''
        write(1,*)'=============== Estimation Terminal 2 ================='
        write(1,*)'========= typeof = (0:Splines, 1:Piecewise, 2:Weibull) ',typeof0    
        write(1,*)'========= number of parameters ',np4
        write(1,*)'========= number of subjects',ng0
        write(1,*)'========= number of groups ',ng0
        write(1,*)'Convergence Criteria istop dc2',istop
        write(1,*) 'Terminal 2 Initialized Parameters:', b04
        write(1,*)''
        if(typeof == 0)then
            if(irep.eq.0)then
                kappaCV(4)=k0s(1) ! kappa branche dc
            else
                kappaCV(4)=kappa1
            endif
            write(1,*)'Smoothing parameter DC ',kappaCV(2)
            write(1,*)'K0s ',k0s
            write(1,*)'irep',irep
            write(1,*)'kappaCV',kappaCV
        endif
        close(1) 
    endif
...
! Recurrent 1: nt0,nt1
        ...
        open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
        write(1,*)'multiveJoint.f90:: Date grid for recurrent 1: date = '
        write(1,*) date
        write(1,*)'multiveJoint.f90:: Date indexes (nt0) for recurrent 1 = '
        write(1,*) nt0
        write(1,*)'multiveJoint.f90:: Date indexes (nt1) for recurrent 1 = '
        write(1,*) nt1
        close(1) 
...
! (10) END Organize Hessian Matrix
!-----------------------------------------------------------
...
! (14) Deallocating All variables
    ...
    close(1) 
    deallocate(b01,b02,b03,b04,b02joint)
