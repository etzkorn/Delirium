...
!TEST
        ...
        !b(1) = dsqrt(1.0d0) !Shape Recurrent
        !b(2) = dsqrt(0.5d0) !Scale Recurrent
        !b(3) = dsqrt(1.0d0) !Shape Terminal
        !b(4) = dsqrt(1.0d0) !Scale Terminal
        !b(5) = dsqrt(0.5d0) !Frailty variance
        !b(6) = b(6) + 0.1d0 ! Alpha
        !b(7) = 0.0d0 ! Covar Recurrent
        !b(8) = 0.0d0 ! Covar Terminal
!!!!!!!!!!!!!!!!!1
...
!     (INVERSION OF A POSITIVE DEFINED SYMMETRICAL MATRIX)
!
!     MATRICE = TRANSPOSEE(T)*T
...
! (TABLE CONTAINING THE UPPER PART OF THE MATRIX TO BE INVERTED
...
!           COMME NUL
!           THRESHOLD OF TOLERANCE BELOW WHICH A PIVOT IS CONSIDERED LIKE NO
!
!     IER : CODE D'ERREUR
!         IER=0 PAS D'ERREUR(No mistake)
!         IER=-1 ERREUR SUR LA DIM.N OU MATRICE PAS DEFINIE POSITIVE
!            (ERROR ON DIM.N OR MATRIX NOT POSITIVE Definite)
!         IER=1 PERTE DE SIGNIFICANCE, LE CALCUL CONTINUE
!            (Loss of significance, The calculation continues)
    implicit none
...
!  1: convergence criteria satisfied (prm=ca, vraisblce=cb, derivee=dd)
!  2: number of maximum iterations attained
!  4: error
...
!add additive
    ...
    double precision,dimension(4)::zero
    !zero is an empty vector of zeros we use for non-penalized splines
    integer::nql,ii,nfmax,idpos,ncount,id,jd,m1,j,i,ij,k
    double precision,dimension(m*(m+3)/2)::fu,v1,vnonpen
    double precision,dimension(m)::delta,b1,bh
    double precision::da,dm,ga,tr
...
!         write(6,*) 'ca =',ca,' cb =',cb,' dd =',dd
        ...
        !!!!!!!!!!!!!!!!!!!!!
        ! TEST: Evaluate likelihood over a grid instead of optimizing
        !b(1) = dsqrt(1.0d0) !Shape Recurrent
        !b(2) = dsqrt(0.5d0) !Scale Recurrent
        !b(3) = dsqrt(1.0d0) !Shape Terminal
        !b(4) = dsqrt(1.0d0) !Scale Terminal
        !b(5) = dsqrt(1.0d0) !Shape Terminal
        !b(6) = dsqrt(1.0d0) !Scale Terminal
        !b(7) = dsqrt(0.5d0) ! Variance
        !b(8) = b(8)+ 0.05d0!alpha1
        !b(9) = 0.0d0 !alpha2
        !b(10) = 0.0d0 ! covariates
        !b(11) = 0.0d0
        !b(12) = 0.0d0
...
!------------------------------------------------------------
    ...
    ! (2b) Calculate log likelihood changing each parameter by small constant "-th"
    do i=1,m
        ll=ll+1
        vaux=fctnames(b,m,i,thn,i0,z,k0)
                if(vaux.eq.-1.d9) then
    ...
    ! (3) approximate the Score for each parameter by taking a scaled difference of (2a) and (2b)
        vl=(fcith(i)-vaux)/(2.d0*th)
        v(ll)=vl
        do j=1,i
            k=k+1
    ! (4) approximate derivative with respect to other parameters
            v(k)=-(fctnames(b,m,i,th,j,th,k0)-fcith(j)-fcith(i)+rl)/th2
        end do
    end do
    open(1, file = '../package_tests/multiv_model_progress.dat',position="append")  
