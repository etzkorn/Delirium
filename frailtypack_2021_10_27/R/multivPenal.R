...
# Format Model Tables
 ...
 if(jointGeneral == F & hazard == "Weibull"){
  	...
  	Parameter = c("Shape, Recurrent", "Scale, Recurrent",
  		  ...
  		  paste0("Beta: Recurrent, ", 1:nbvar[1]),
  		  paste0("Beta: Terminal1, ", 1:nbvar[2]),
  		  paste0("Beta: Terminal2, ", 1:nbvar[4]))
  	...
  	Parameter = c("Shape, Recurrent", "Scale, Recurrent",
  		  "Shape, Terminal1", "Scale, Terminal1",
  		  "Shape, Terminal2", "Scale, Terminal2",
  		  "Sigma2, Terminal1", "Sigma2, Terminal2", "Rho",
  		  "Alpha, Terminal1", "Alpha, Terminal2",
  		  paste0("Beta: Recurrent, ", 1:nbvar[1]),
  		  paste0("Beta: Terminal1, ", 1:nbvar[2]),
  		  paste0("Beta: Terminal2, ", 1:nbvar[4]))
  }else if(jointGeneral == F & hazard == "Splines"){
  	f <- function(b){
  		c(exp(b[1:6]),
  		  exp(b[7]),
  		  b[(np-nvar-1):np])
  ...
  ans$H_hessOut <- matrix(ans$H_hessOut, nrow = np, ncol = np)
  ans$varH.Estimate <- f.prime(ans$b) %*% ans$H_hessOut %*% f.prime(ans$b)
  ans$summary.table <- tibble(
  	Parameter = Parameter,
  	Raw = ans$b,
  	Raw.SE = sqrt(diag(ans$H_hessOut)),
  	Estimate = f(ans$b),
