...
$$
    ...
    \gamma_r = 0.5 , \gamma_1 = 0 \text{(treatment effects)}\\
    \eta_r = 0.5, \eta_1 = 1 \text{(weibull scale)}\\
    \beta_r = 1, \beta_1 = 1 \text{(weibull shape, exponential)}\\
...
expand.grid(theta = c(0.5, 2),
	...
	Parameter = c("betaR", "etaR",
		...
		"trtR", "trtD")) %>%
mutate(truth = rep(c(1,0.5, 1,1,-1,-1,0.5,0), each = 4),
...
####################################################################
# Generate Data
if(!file.exists("JointModelsA_120720.rdata")){
data0 <- 
map(par0, 
    ~simulate.joint.data(n = 500,truncate = 2,par0 = .))
...
~frailtyPenal(formula = Surv(g, event) ~ trt + cluster(id) +
	...
	maxit = 100,
	RandDist = "LogN"))
save(data0, mod.joint.a, file = "JointModelsA_120720.rdata")
} else load(file = "JointModelsA_120720.rdata")
...
mutate(par0 = map(par0, ~tibble(par0 = .))) %>%
unnest(c(est,par0)) %>%
select(-fits)
...
### Notes on the weibull gap model (parametric)

* The failure % is high for higher values of theta (2 versus 0.5).

* Estimates of betaR and theta are slightly biased for higher values of theta.

* The number of events/person that were produced when theta = 2 were extremely high (up to 150), so theta = 2 might represent an extreme value for the frailty variance.

* The average estimated standard error was lower than the empirical standard errors for multiple parameters when theta = 2.

# Allow for Smooth Hazard Function

```{r, fig.height = 8, fig.width = 12, echo = FALSE}
if(file.exists("../package_tests/JointModelsB_120720.rdata")){
load("../package_tests/JointModelsB_120720.rdata")
}else{
mod.joint.b <-
map(data0, 
~frailtyPenal(formula = Surv(g, event) ~ trt + cluster(id) +
...
save(mod.joint.b, file = "../package_tests/JointModelsB_120720.rdata")
}
results <- 
tibble(data = 1:400,
       par0 = par0,
...
### Notes on the weibull gap model (spline fit)

* The failure % is lower for both values of theta compared to the parametric model.

* Alpha seemed slightly biased when theta = 2.

* We don't see the same degree of bias for betaR and theta with the spline model.
