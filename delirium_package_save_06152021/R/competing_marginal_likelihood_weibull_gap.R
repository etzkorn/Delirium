...
#'
#' @description Calculate the marginal (negative) log likelihood of a parameter combination for the weibull gap model with two competing terminal events.
#'
...
competing.marg.likelihood.weibull.gap <- function(phi, data, covar.names, GH = 30){
	names(phi) <- c(paste0(rep(c("beta","eta"),3),rep(c("R","D","D2"), each = 2)),
		    "theta", "alpha1","alpha2",
	...
	data$integral = log(as.vector(exp(cll) %*% (rules$weights * exp(rules$nodes^2))))
	data$integral = ifelse(!is.finite(data$integral),
		           log(10^-10),
		           data$integral)

	-sum(data$hazR + data$hazD + data$hazD2 + data$integral -
	    	0.5*log(2*pi) - 0.5*log(phi["theta"]))
}
