...
!Initialize First Terminal Event Model: DC
        ...
        allocate(str00(ng0))
        str00=1
        allocate(H_hess0(np2joint,np2joint),HIH0(np2joint,np2joint),zis(nzsha+6))
!         allocate(x1Outs(mt2),x2Outs(mt2),lamOuts(mt2,3),lam2Outs(mt2,3))
        ...
        ag0,noVar2,maxit00,irep,np2,b02,&
        H_hess0,HIH0,resOut,LCVs, &
        xTOuts,lamTOuts,xSuTs,suTOuts,typeof0,equidistant0,nbinterv0,mt2, &    
...
!        write(*,*) "b02joint = ", b02joint

        !Initialization using recurrent event 1
!        call joint(nsujet0,ngrp0,str00,0,nzsha,kappa1,&
!        tt00,tt10,ic0,groupe0,groupe0,0, & ! recurrent event 1 data
!        tt0dc0,tt1dc0,icdc0,0,icdc0,& ! terminal event 1 data
!        nva10,vax0,nva20,vaxdc0,vaxdc0, noVarEvent(1:2), 1, maxit00, &
!        np2joint,b02joint,&
!        H_hessOut,HIHOut,resOut,LCVs,&
!        x1Out,lamOut,xSu1,suOut,& ! recurrent event
!        x2Out,lam2Out,xSu2,su2Out, & ! terminal event
!        equidistant0,nbintervDC0, mt2, &
!        cpts, & !counts
!        ier,paraweib, & 
       !double precision,dimension(2*(ngrp(3)+1)),intent(out)::paraweib !IJ
      !!paraweib =c(shapeweib(1),shapeweib(2),scaleweib(1),scaleweib(2))
!        frailtyEstimates, &
!        linearpred,linearpreddc,zis,&
!        time,timedc, & 
!        linearpreds,&
!        1,& !typeJoint0 = 1
!        0,& !intcens0 = 0
!        indices,ttUs, ordretmps, &
!        1, & ! initialize = 1
!        1,& ! 0 = gamma frailties, 1 = lognormal frailties 
!        paratps,filtretps0Mul,BetaTpsMatMul,BetaTpsMatDc, & ! Time dependent stuff all set to 0
!        EPS, 20)
        !counts(1) = ni
        !counts(2) = cpt
        !counts(3) = cpt_dc
...
!        write(*,*) "b02joint = ", b02joint
        ...
        deallocate(filtretps0Mul)
        deallocate(H_hess0,HIH0,zis,str00)!,b02joint)
!         deallocate(x1Outs,x2Outs,lamOuts,lam2Outs)
        deallocate(xTOuts,lamTOuts)!en plus
        deallocate(linearpreds,martingaleCoxs,times)
...
!         deallocate(x1Outs,x2Outs,lamOuts,lam2Outs)
        ...
        !write(*,*)''
        !write(*,*)'=============== Estimation Terminal 2 ================='
        !write(*,*)'========= typeof = (0:Splines, 1:Piecewise, 2:Weibull) ',typeof0    
        !write(*,*)'========= number of parameters ',np4
        !write(*,*)'========= number of subjects',ng0
        !write(*,*)'========= number of groups ',ng0
        !write(*,*)'Convergence Criteria istop dc2',istop
        !write(*,*) 'Terminal 2 Initialized Parameters:', b04
        !write(*,*)''
        if(typeof == 0)then
            if(irep.eq.0)then
