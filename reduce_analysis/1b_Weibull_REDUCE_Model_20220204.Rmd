...
```{r, echo = FALSE,message = FALSE}
if(!file.exists("../reduce_analysis_output/REDUCE_Weibull_Models_Covariates_20220214.rdata")){

# the death initialization model for the gap.model doesn't work, so we set some starting vals
gap.model1 <-
frailtyPenal(formula = Surv(gap, del) ~ treatment+ apache +
		terminal(death) + cluster(id),
	formula.terminalEvent = ~treatment,
	data = df2, 
	recurrentAG = F,
...
# GAP Time Models (with/without initialization)
gap.model <-
multivPenal(formula = Surv(tstart, tstop, del) ~ treatment + apache +
		terminal(death) + terminal2(discharge) + cluster(id),
	formula.terminalEvent = ~treatment+ apache,
	formula.terminalEvent2= ~treatment+ apache,
	data = df2,
	gapTimes = T,
	jointGeneral = F,
	hazard = "Weibull",
...
multivPenal(formula = Surv(tstart, tstop, del) ~ treatment +
	...
	initialize = T)
save(gap.model, cal.model,
     file = "../reduce_analysis_output/REDUCE_Weibull_Models_Covariates_20220214.rdata")
}else load("../reduce_analysis_output/REDUCE_Weibull_Models_Covariates_20220214.rdata")




frailtyPenal(formula = Surv(tstart, tstop, del) ~ treatment + apache +
		terminal(death) + cluster(id),
